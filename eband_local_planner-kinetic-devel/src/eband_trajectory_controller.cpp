// 第四部分是将局部规划转成速度（cmd_vel或twist_cmd）控制指令
...
namespace eband_local_planner{
  ...
  // 这个函数就是处理差速模型,把速度放到twist_cmd,如果到达目的地goal_reached就置0
  // 这函数里面会执行三步(三步都是相斥的)
    // 第一步,判断是否到达xy目标
  ...
  bool EBandTrajectoryCtrl::getTwistDifferentialDrive(geometry_msgs::Twist& twist_cmd, bool& goal_reached) {
    ...
    // 获取机器人框架中前两个气泡之间的差异
    // 将frame1和frame2的姿势转换为参考frame，并获得frame1和frame2的差
    // 然后把差放到bubble_diff,也就是返回值
    ...
    // 我们需要检查我们是否在最终目的地的阈值内,如果没有到达目的地(xy距离),那么就不用管这个
    if (!command_provided) {
      ...
      // position
      // 在开始最后旋转之前，比公差要求更接近目标。最后转弯可能会导致您稍微偏离位置
      // 就是xy已经到达目的地了
      if((fabs(bubble_diff.linear.x) <= 0.6 * tolerance_trans_ &&
        ...
        if (fabs(orientation_diff) > tolerance_rot_) {
          ...
          robot_cmd.angular.z =
            // 最小到位角速度。min_in_place_vel_th_一般为0,没懂啥意思
    ...
    // 检查1 -检查机器人当前的姿势是否与下一个泡泡太不一致,不一致就原地旋转
    // 如果一致(就是两个方向是否一致,不太懂),就不用管
    if (!command_provided) {
      ...
      if (fabs(bubble_diff.angular.z) > in_place_rotation_threshold) {
        ...
        // min_in_place_vel_th_没懂这个意思,这个一般设置为0
    ...
    // 如果到达此处，则意味着我们需要使用PID控制器移至下一个气泡
    // 就是说既没有到达目标,也没有机器人气泡与下一个气泡相差太大,那么就正常执行这个
    if (!command_provided) {
      ...
      // 速度限制
      linear_velocity = std::min(linear_velocity, last_x_+ forward_sign*acc_max_trans_/ctrl_freq_);
